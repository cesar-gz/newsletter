...
export class SubscribeFormComponent {
  ...
  onSubscribe(form: NgForm) {
    if (form.valid) {
      ...
      this.http.get<{ userId: number }>('https://newsletter.api.cesargz.com/user/lastUserId').subscribe(
        (response) => {
          ...
          this.http.post('https://newsletter.api.cesargz.com/users/subscribe/', jsonData).subscribe(
            (response) => {
              //console.log('API Response:', response);
              form.resetForm();
  ...
  unsubscribe(form: NgForm) {
    if (form.valid) {
      ...
      this.http.get<{ userId: number }>(`https://newsletter.api.cesargz.com/users/id/${this.formData.name}/${this.formData.email}`).subscribe(
        (response) => {
          var deleteUserId: number;
          deleteUserId = response.userId

          this.http.delete(`https://newsletter.api.cesargz.com/users/unsubscribe/${deleteUserId}`).subscribe(
            (response) => {
              //console.log('API Response:', response);
              form.resetForm();
